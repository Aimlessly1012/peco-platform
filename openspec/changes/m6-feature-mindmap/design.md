...
### D4: 业务流程图 = L4 业务流驱动的小 flowchart（用户追加）

- 数据源：L4「核心业务流」条目（2-4 条）+ 各条涉及功能域的 L3「关键流程」
- 生成：单次 LLM 调用输出全部流程图（每条业务流一张 `flowchart TD`，节点为业务步骤——用户动作/系统行为，≤8 节点，节点文案 ≤12 字，禁文件名/函数名）；复用既有 mermaid 启发式校验 → 重试 1 次 → fallback_text（业务流原文）
- 不缓存（依赖 L4，每次 deep 重算，1 次调用成本可忽略）；fast 模式跳过（无精 L3/L4）
- 存 business_flows_json：[{title, mermaid, fallback_text}]
- 页签定位随之敲定：报告页签 = 需求视角（功能导图 + 业务流程图 + 需求文档 + 时序图）；功能地图页签 = 代码视角（结构导图 + 文件子导图 + **模块数据流图**——自报告页签移入）
