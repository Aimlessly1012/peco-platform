...
### D5: 功能域业务归组（用户二次反馈：不要平铺，要逐层树状）

- 功能域 >8 个时增加「业务组」层级：`# 产品 → ## 业务组 → ### 功能域 → - 功能点` 四层；≤8 个保持三层不归组
- 归组方式：单次 LLM 小调用——输入全部功能域（名称 + kind + L3 业务目标首句），输出 JSON `{组名(中文业务词): [功能域名...]}`；程序化校验：成员必须真实存在、不重复，遗漏项归入「其他」组，单组即全部时视为归组失败
- 失败/低质量降级：保持现平铺三层（不阻塞）
- 缓存：按功能域名称集合 hash（模块结构没变就复用归组结果）
- markmap 初始展开层级相应调整：四层树展开到业务组层，逐层点开——"逐层"体验由归组层级 + 折叠交互共同达成

### D6: 前端页面逻辑导图（用户追加）

- 新增报告件「页面结构导图」page_map_markdown（alembic 新列）：按 page 模块的路由 path 段程序化建树——`# 产品 → ## 一级路由段 → ### 页面（route path）→ - 页面逻辑要点`
- 页面逻辑要点：优先取该页面所属功能域的功能点（B2 产物），无则取页面入口文件 L2 摘要压缩首句；纯程序化拼装零额外 LLM
- fast 模式同样生成（要点退化为入口文件名清单）；前端复用 MarkmapView 作为功能导图的姊妹卡片
