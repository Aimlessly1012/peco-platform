...
- [ ] 8.4 确认 `generated.ts` 已入库，且 diff 中字段变化可读
- [ ] 8.5 脚本 `--check` 模式追加结果验证：`OVERRIDES` / `SIGNATURES` 中人写 N 条，产物中必须命中 N 条，未命中逐条列出并非零退出。补的是 R5 那条缝——现有守卫只查「在不在」，对「取到了错的那一份」零覆盖，本 change 期间已被踩中两次（Set 遮蔽、优先级反转前的 4 条失效）
