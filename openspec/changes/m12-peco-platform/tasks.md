...
- [ ] P7 RAG 前端页面迁入 `/rag/*`：项目列表、项目详情三页签（含 markmap/mermaid/进度条）、聊天页（SSE + 引用联动）、MCP 说明页；只统一令牌，不改结构与交互
- [ ] P8 平台侧 `/rag/api/*` 代理：签发内部令牌转发后端；**重点验证 SSE 流式（聊天 token、索引进度）与 MCP 长连接不被代理破坏**
- [ ] B1 RAG 后端接受内部令牌：共享密钥验签 + 校验 status/禁用态；迁移期保留密码登录作为退路
- [ ] B2 users 表对齐：加 github_id、去 password_hash 依赖；chat_sessions.user_id 关系不变
- [ ] V2 阶段二验收：RAG 全功能回归（录入/索引进度/报告各图/聊天流式/引用联动/MCP 接入）；未批准与被禁用用户被挡
