...
import CopyButton from "@/components/rag/CopyButton";
...
import { buildModuleMindmap, MODULE_MINDMAP_FILE_LIMIT } from "@/lib/rag/mermaid";
...
const MermaidDiagram = dynamic(() => import("@/components/rag/MermaidDiagram"), {
  ssr: false,
  loading: diagramLoading("LOADING DIAGRAM…"),
});

/** markmap 同理（markmap-lib + markmap-view + d3，只在需要时进包）。 */
const MarkmapView = dynamic(() => import("@/components/rag/MarkmapView"), {
  ssr: false,
    ...
    <div className="flex min-h-0 flex-1">
      ...
      <aside className="hidden w-[212px] flex-none flex-col gap-6 overflow-y-auto border-r border-line bg-canvas px-5 py-6 md:flex">
        <div className="flex flex-col gap-2">
          <Link href="/rag" className="text-[11px] text-faint hover:text-ink">
            ← 项目
          </Link>
        ...
        <div className="flex flex-col gap-2">
          <Link
            href={`/rag/projects/${projectId}/chat`}
