 ...
 * RAG 后端基址。
 *
 * 平台把后端挂在同域的 /rag/api 下（nginx 剥前缀转 FastAPI），所以默认是相对路径，
 * 浏览器请求自动带上 NextAuth 的 cookie——后端用共享密钥验签即可（M12 D2）。
 * 本地开发若后端跑在别的端口，用 NEXT_PUBLIC_RAG_API_BASE 覆盖。
 */
export const API_BASE =
  process.env.NEXT_PUBLIC_RAG_API_BASE || "/rag/api";
...
/**
 * 401 的统一去向由页面注入（平台的登录页在 /login，由 next-auth 管）。
 * api 层只负责通知，跳哪、怎么带回跳地址交给上层。
 */
...
/** 登录态由平台统一管理，RAG 后端不再有 /auth/* 路由；保留判断以防误触发跳转。 */
